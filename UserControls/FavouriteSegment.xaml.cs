...
{
    /// <summary>
    /// Логика взаимодействия для FavouriteSegment.xaml
    /// </summary>
    public partial class FavouriteSegment : UserControl
    {
        private UnitOfWorkContent unit;
        public FavouriteSegment()
        {
            InitializeComponent();
            DataContextChanged += FavouriteSegment_DataContextChanged;
            unit = new UnitOfWorkContent();
        }

        private void FavouriteSegment_DataContextChanged(object sender, DependencyPropertyChangedEventArgs e)
        {
            var dataContext = DataContext as FavouriteModel;
        ...
        private void DeleteFromFavouriteBtn_Click(object sender, RoutedEventArgs e)
        {
            var favourite = DataContext as FavouriteModel;
            unit.Favourites.DeleteByFavourite(favourite);
        }
    }
}
